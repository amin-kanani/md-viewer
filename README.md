...
>
> Detecting this means type-checking a small SwiftUI probe against each candidate SDK, which compiles SwiftUI's module interface. The first build on a machine therefore spends around a minute in `==> Probing …`; the result is cached under `.build/`, so later builds finish in a couple of seconds.
